...
def combat_loop(player_stat_input):  # pass monster info and player info?
    ...
    mob_choice = choice(list(mobs.items()))
    print(mob_choice)
    mob_stats = {
        "Name": mob_choice[0],
        "HP": mob_choice[1][0],
        "Attack": mob_choice[1][1],
        "Defence": mob_choice[1][2],
        "Special": mob_choice[1][3],
        "Agility": mob_choice[1][4],
        "Intelligence": mob_choice[1][5],
        "Luck": mob_choice[1][6],
    }
    is_over = False
    while ~is_over:
...
if __name__ == '__main__':
    ...
    while player_alive:
        ...
        if direction == 'forwards' or 'forward':
            position += 1
